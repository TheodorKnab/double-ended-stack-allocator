...
int main()
{
	// You can add your own tests here, I will call my tests at the end with a fresh instance of your allocator and a specific max_size
	{
		...
		Tests::Test_Case_Success("Allocate() does not return nullptr", Tests::VerifyAllocationSuccess(allocator, 32, 5));
		Tests::Test_Case_Success("AllocateBack() does not return nullptr", Tests::VerifyAllocationBackSuccess(allocator, 32, 5));
		Tests::Test_Case_Success("Free() resets to the correct address", Tests::VerifyFreeSuccess(allocator, 32, 5));
		Tests::Test_Case_Success("FreeBack() resets to the correct address", Tests::VerifyFreeBackSuccess(allocator, 32, 5));
	}
